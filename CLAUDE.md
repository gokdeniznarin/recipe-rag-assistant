...
- **🔴 KURU ↔ PİŞMİŞ tahıl karışıklığı (bilinçli ertelendi, 2026-07-27).** Kuru tahıl pişerken su emiyor: 100 g kuru arpa = 354 kcal, 100 g pişmiş arpa = 123 kcal (**2.9×**). Veritabanı gıdayı **satıldığı** hâliyle indeksliyor (kuru), fotoğraf ise **tabağı** gösteriyor (pişmiş). Gerçek fotoğrafta ölçüldü: `barley 80 g → 283 kcal`, gerçeği ~98 — ve o öğe tabak toplamının **%43'üydü**.
  - **Sebep bizim kuralımız:** parantez cezası (−10) `(Canned)` için kondu ve doğruydu, ama `(Cooked)` fotoğrafın tam da gösterdiği şey. Puanlar: `Barley` (kuru) **8**, `Pearled Barley (Cooked)` **−7**. Yani doğru cevap, doğru olduğunu gösteren tek işareti yüzünden cezalandırılıyor.
  - **Prompt'la ÇÖZÜLMÜYOR** (ölçüldü): `cooked barley` sorgusu da kuru `Barley` döndürüyor — "cooked barley" hiçbir adayla birebir eşleşmediği için tam-eşleşme bonusu kimseye gitmiyor, karar parantez cezasına kalıyor.
  - **Sınır DAR:** yalnızca pişmiş varyantın *parantez içinde* ifade edildiği gıdalarda çıkıyor. `white rice`→129 ✅, `lentils`→`Cooked Lentils` ✅ ("Cooked" ana adın içinde, ceza yok), `farro`→133 ✅; `barley` ❌.
  - **Çözüm hazır ama uygulanmadı:** dar bir "kuru satılıp pişmiş yenen" listesi (rice, barley, pasta, lentils, beans, quinoa, oats, bulgur, couscous, farro, spelt) ve o sınıfta `(Cooked)`'ı cezalandırmak yerine ödüllendirmek (+15 ile `Barley (Cooked)` **13** > kuru **8**). **Genel kural OLMAMALI:** "pişmişi tercih et" demek `carrots` için `Cooked Carrots (Fat Added in Cooking)` seçtirirdi. Fiziksel gerekçe yalnızca kuru gıdalar için geçerli — kimse çiğ makarna yemiyor.
